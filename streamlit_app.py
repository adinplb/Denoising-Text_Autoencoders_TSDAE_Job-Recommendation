...
JOB_DETAIL_FEATURES = [ # Using this name as per last user prompt
...
def sbert_onet_finetuning_page():
    ...
    if st.session_state.get('onet_data') is None:
        with st.spinner("Loading O*NET standard occupation data..."):
            # Use the corrected constant name JOB_DETAIL_FEATURES
    ...
    if onet_df is None:
        st.error("Failed to load O*NET data. Cannot proceed with SBERT O*NET fine-tuning."); return
    ...
    st.info("Preparing O*NET data for fine-tuning (preprocessing texts)...")
    ...
    if valid_onet_entries.empty:
        st.error("No valid processed text entries found in O*NET data."); return
    
    st.info(f"Using {len(valid_onet_entries)} O*NET entries for matching.")

    sbert_onet_epochs = st.number_input("Epochs for SBERT O*NET fine-tuning:", min_value=1, max_value=10, value=1, key="sbert_onet_epochs_v2")
    sbert_onet_batch_size = st.selectbox("Batch size for SBERT O*NET fine-tuning:", options=[4, 8, 16, 32], index=2, key="sbert_onet_batch_size_v2")
    num_positive_matches_per_job = st.number_input("Number of top O*NET matches to use as positive examples per job:", min_value=1, max_value=5, value=1, key="sbert_onet_top_n_v2")

    if st.button("Start SBERT O*NET Fine-tuning", key="start_sbert_onet_ft_btn_v2"):
        with st.spinner("Preparing training data by matching jobs to O*NET and then fine-tuning... This may take a while."):
            try:
                job_texts_to_match = main_job_data['processed_text'].fillna('').astype(str).tolist()
                job_texts_to_match = [text for text in job_texts_to_match if text.strip()]
                
                if not job_texts_to_match:
                    st.error("No processed job texts available from main dataset to perform matching for fine-tuning."); return

                onet_texts_for_embedding = valid_onet_entries['processed_onet_text'].tolist()
                
                model_state_ind = f"tsdae:{st.session_state.model_trained_flags['tsdae_trained_this_session']}_cvjob:{st.session_state.model_trained_flags['cv_job_finetuned_this_session']}"

                current_job_embeddings = generate_embeddings_with_progress(bert_model_to_ft, job_texts_to_match, model_state_indicator=f"{model_state_ind}_jobtexts_sbert_onet")
                onet_embeddings = generate_embeddings_with_progress(bert_model_to_ft, onet_texts_for_embedding, model_state_indicator=f"{model_state_ind}_onettexts_sbert_onet")

                if current_job_embeddings.size == 0 or onet_embeddings.size == 0:
                    st.error("Failed to generate embeddings for jobs or O*NET data."); return

                train_examples_sbert_onet = []
                similarity_matrix_job_onet = cosine_similarity(current_job_embeddings, onet_embeddings)

                for i in range(len(job_texts_to_match)):
                    job_text_anchor = job_texts_to_match[i]
                    top_n_onet_indices = np.argsort(similarity_matrix_job_onet[i])[::-1][:num_positive_matches_per_job]
                    for onet_idx in top_n_onet_indices:
                        positive_onet_text = onet_texts_for_embedding[onet_idx]
                        train_examples_sbert_onet.append(InputExample(texts=[job_text_anchor, positive_onet_text]))
                
                if not train_examples_sbert_onet:
                    st.error("No training examples could be created for SBERT O*NET fine-tuning. Check data or matching logic."); return

                st.write(f"Created {len(train_examples_sbert_onet)} (job_text, matched_onet_text) training pairs.")
                
                train_dataloader_sbert_onet = DataLoader(train_examples_sbert_onet, shuffle=True, batch_size=sbert_onet_batch_size)
                train_loss_sbert_onet = losses.MultipleNegativesRankingLoss(model=bert_model_to_ft)
                warmup_steps_sbert_onet = int(len(train_dataloader_sbert_onet) * sbert_onet_epochs * 0.1)

                st.info(f"Starting SBERT O*NET fine-tuning for {sbert_onet_epochs} epoch(s)...")
                bert_model_to_ft.fit(
                    train_objectives=[(train_dataloader_sbert_onet, train_loss_sbert_onet)],
                    epochs=sbert_onet_epochs, warmup_steps=warmup_steps_sbert_onet,
                    output_path=None, show_progress_bar=True
                )
...
# ... (bert_model_page, clustering_page, upload_cv_page, job_recommendation_page, annotation_page, _calculate_average_precision, evaluation_page as before)
# Ensure these functions are fully defined as in the last complete version.
def bert_model_page():
...
def clustering_page(): # Placeholder
    ...
    st.info("Clustering page (full implementation as before)")
    return

def upload_cv_page(): # Placeholder
    st.header("Upload & Process CV(s)")
    # ... (Full implementation from previous version)
    st.info("Upload CV page (full implementation as before)")
    return

def job_recommendation_page(): # Placeholder
    st.header("Job Recommendation")
    # ... (Full implementation from previous version)
    st.info("Job Recommendation page (full implementation as before)")
    return
    
def annotation_page(): # Placeholder
    st.header("Annotation of Job Recommendations")
    # ... (Full implementation from previous version, including CSV upload)
    st.info("Annotation page (full implementation as before, with CSV upload)")
    return
...
def evaluation_page(): # Placeholder
    ...
    # ... (Full implementation from previous version, with per-CV table and averages)
    st.info("Evaluation page (full implementation as before, with per-CV table and averages)")
    return
